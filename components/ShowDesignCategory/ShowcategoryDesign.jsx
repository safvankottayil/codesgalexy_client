...
const getData = async (category) => {
    const res = await fetch(userUrl + '/showCategoryDesigns/' + category,{next:{revalidate:10}})
    const repo = await res.json()
    console.log(repo);
    return repo
}

async function ShowDesign({ category }) {

    const data = await getData(category)
    console.log(data);
    return (
        <div className='m-2 md:m-4 flex flex-col w-full'>
            ...
            <div className='w-full grid gap-1 grid-cols-1 md:grid-cols-2 xl:grid-cols-3'>
                {data.Designs.map((item) => {
                    ...
                    return (
                        <>
                            <div key={item._id} className='bg-slate-100 rounded-sm   h-fit '>
                                ...
                                <div className='flex justify-between items-center w-full '>
                                    <div className='flex'>
                                        ...
                                        <div className='flex flex-col pl-2 min-w-[100px] '>
                                            <p className='text-lg pl-2 w-[130px] 2xl:max-w-[200px] font-bold capitalize truncate'>{item.title}ekfhiuegfuhegufgiyuegfuyegyfgyegfyuegfyugeyfgegfyegfygegfyg</p>

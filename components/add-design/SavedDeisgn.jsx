...
function SavedDesign({id}) {
    useEffect(()=>{
      userAxios.get('/saveDesign?id='+id,{headers:{Authorization:Token}}).then((res)=>{
        if(res.data.status){
            Htmlmount(res.data.Design.html)
            Cssmount(res.data.Design.css)
            jsmount(res.data.Design.js)
           setTimeout(()=>{
              click.current.click()
           },1000/2)
        }else{
    ...
    const [jsvalue, jsmount] = useState('')
    const [Htmlvalue, Htmlmount] = useState('')
    const [Cssvalue, Cssmount] = useState('')
    //Html code
    ...
    function Submit() {
        if (!tages.length < 3) {
          ...
          if(check){
            userAxios.patch('/add-design', { Htmlvalue,id, jsvalue, Cssvalue, tages,title:title.current.value,category:category.current.value.toLowerCase()},{headers:{Authorization:Token}}).then((res) => {
                if (res.data.status) {
    ...
    function Save(){
        alert(1)
        userAxios.patch('/saveDesignUpdate',{Htmlvalue,jsvalue, Cssvalue,id},{headers:{Authorization:Token}}).then((res)=>{
            if(res.data.status){
    ...
    return (
        <>
            ...
            <div className='grid md:grid-cols-2 md:h-[600px] grid-rows  px-2 pt-1 rounded-xl'>
                <div className='bg-black border-2 h-[300px] md:h-[580px]  border-gray-200  md:rounded-s-2xl '>
                    <div className='h-12 bg-slate-900 rounded-ss-xl'>
                        <div className=' flex h-12 items-end'>
                            ...
                            <button ref={click} onClick={() => setShow(CssEditor)} className='bg-zinc-700 text-white hover:bg-slate-500 flex   rounded-t-md p-1 px-3 ml-2' ><FaCss3Alt className='w-6 h-6 ' /> <span>CSS</span></button>
                ...
                <div className='md:col-span-2 h-16  rounded-md mt-1  bg-blue-500'>
                    <div className='flex items-center h-16 justify-end'>
                        <DesignRemove id={id} />
                        <button onClick={()=>Save()} className='p-2 ml-2 bg-emerald-950 mr-2 rounded-md text-white'>Update</button>

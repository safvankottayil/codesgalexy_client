...
function ShowDesignPosts() {
  ...
  useEffect(() => {
    setLoder(true);
    UserAxios.get("/profile/posts", { headers: { Authorization: token } }).then(
      (res) => {
        if (res.data.status) {
       ...
       { Design[0]? (
          Design.map((Data) => {
            ...
            return (
              <div
                key={Data._id}
                className="bg-slate-50 border-2 border-grey-200 "
              >
                ...
                <div className="flex justify-between items-center w-full ">
                  <div className="flex">
                    <div className="w-14">
                      {Data.UserId.image ? (
                        <img
                          className="bg-black rounded-full ml-2 mb-2 h-14 w-14"
                    ...
                    <div className="flex flex-col pl-2 min-w-[100px] ">
                      <p className="text-lg pl-2 w-[130px] 2xl:max-w-[200px] font-bold capitalize truncate">
                        {Data.title}
                      </p>
                      <p className="text-md pl-2 font-semibold capitalize">
                        {Data.UserId.name}
                      </p>
                  ...
                  <div className="flex items-center ">
                    ...
                    <Link
                      ...
                      href={`/profile/posts/${Data._id}`}
                      className="bg-black h-8 font-bold text-sm mr-2 rounded-md p-2 text-white"
                    >
                      Show
                    </Link>
                  </div>

...
function Paragraphs({ data, Update, index,Delete,swap }) {
  ...
  useEffect(() => {
    ...
    Update(data.map((value,i)=>{
      if(i==index){
        return {data:value.data,css:css,bg:bg,color:color,type:'para'}
      }else{
        return value
      }
    }))
  ...
  if (!data[index]) {
    ...
    Update([...data, { data: { P: text },css:css,bg:bg,color:color,type:'para', isEdit: true }]);
  }
  ...
  return (
    <div className=" text-justify relative">
        <div style={{background:bg}} className="flex justify-between relative">
      <p style={{overflowWrap:'break-word',color:color}} className={...css}>
      {text}
      </p>
      <div>
      ...
      {ShowEditer? <span className="flex flex-col px-1 z-10 absolute -right-7 bg-gray-400 rounded-md top-9  items-center">

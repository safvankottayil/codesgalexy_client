...
function List({ data, Update, index,Delete,swap }) {
  ...
  useEffect(()=>{
   ...
   useEffect(() => {
     ...
     Update(data.map((value,i)=>{
       if(i==index){
         return {data:value.data,css:css,bg:bg,color:color,type:'list'}
       }else{
         return value
       }
     }))
   ...
   if (!data[index]) {
     ...
     Update([...data, { data: { text: text },css:css,bg:bg,color:color,type:'list', isEdit: true }]);
   }
  ...
  return (
    <div className="text-justify relative ">
      <div style={{background:bg}} className="flex justify-between relative pl-2">
      <div style={{overflowWrap:'break-word',color:color,display:'flex'}} className={...css}>
      <div className="flex flex-col justify-start"><LiaHandPointRight className="mr-2 mt-1 h-6 w-6"/></div>
       <i>{text}</i> 
      </div>
      <div>
      <CiMenuKebab onClick={()=>SetShowEditer(!ShowEditer)} fill="black" className="w-7 mt-2 h-7"/>
      </div>
      {ShowEditer? <span className="flex flex-col px-1 absolute z-10 -right-7 bg-gray-400 rounded-md top-9  items-center">
      ...
      {/* -------------------------------------------------------------------------------------------------- */}
      {editer?
      <div className="bg-slate-200 rounded-md w-72 flex flex-col  z-10 absolute right-0 ">
        <div>
